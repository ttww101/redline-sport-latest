...
#import "NELPPrivatizationConfig.h"
...
#pragma mark - 即将废弃的方法
/**
 @brief    初始化播放器 (即将废弃，请使用 setExternConfigAudioSession: 和 init 替换)
 
 @param isNeed 是否需要内部配置audiosession
 @return 返回播放器实例
 */
- (instancetype)initWithNeedConfigAudioSession:(BOOL)isNeed NS_DEPRECATED_IOS(2_0, 2_0);

/**
 @brief    初始化播放器，输入播放文件路径 (即将废弃，请使用 setExternConfigAudioSession: 和 initWithContentURL:error: 替换)
...
- (id)initWithContentURL:(NSURL *)aUrl
  needConfigAudioSession:(BOOL)isNeed
                   error:(NSError **)error NS_DEPRECATED_IOS(2_0, 2_0);
