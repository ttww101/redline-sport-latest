...
- (instancetype)init {
    ...
    self.backgroundColor = UIColorHex(0xf9f9f9);
    [self _initGroups];
    [self _initTopLine];
...
- (void)_initTopLine {
    ...
    line.backgroundColor = UIColorHex(0xbfbfbf);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = 0; i < curGroupPageCount; i++) {
        ...
        if (page - curGroupPageIndex == i) {
            layer.backgroundColor = UIColorHex(0xfd8225).CGColor;
        } else {
            layer.backgroundColor = UIColorHex(0xdedede).CGColor;
        }
